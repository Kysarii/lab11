...
#бегаем по строчкам и столбикам и проверяем 0 точки#
...
def can_move(mas):
    ...
    for i in range(4):
        for j in range(3):
            if mas[i][j] == mas[i][j + 1]:  # Check horizontally
                return True

    # Check for possible merges in columns
    for j in range(4):
        for i in range(3):
            if mas[i][j] == mas[i + 1][j]:  # Check vertically
                return True
